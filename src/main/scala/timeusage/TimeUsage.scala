...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeeds = columnNames.filter(x => x.contains("t01") || x.contains("t03") || x.contains("t11") || x.contains("t1801") || x.contains("t1803"))
    val workingActivities = columnNames.filter(x => x.contains("t05") || x.contains("t1805"))
    val otherActivities = columnNames.filter(x => x.contains("t02") || x.contains("t04") || x.contains("t06") || x.contains("t07") || x.contains("t08") ||
      x.contains("t09") || x.contains("t10") || x.contains("t12") || x.contains("t13") || x.contains("t14") || x.contains("t15") || x.contains("t16") ||
      x.contains("t18")).diff(primaryNeeds ++ workingActivities)
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy($"working",$"sex",$"age")
      .agg(
        round(avg($"primaryNeeds"),1).as("primary needs"),
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"SELECT working, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, " +
      s"ROUND(AVG(work),1) as work,  ROUND(AVG(other),1) as other FROM $viewName GROUP BY working, sex, age ORDER BY working, sex, age"
